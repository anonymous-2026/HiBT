...
TEST60="${ROOT_DIR}/artifact/data/requests/test_requests_60.json"
HELDOUT12="${ROOT_DIR}/artifact/data/requests/test_requests_heldout_full_12.json"
...
check_path "test60 benchmark" "$TEST60"
check_path "heldout12 benchmark" "$HELDOUT12"
echo
...
if [[ "$BACKEND" == "all" || "$BACKEND" == "local" ]]; then
  ...
  echo "example:"
  echo "  python scripts/run_eval.py --backend local --requests-file artifact/data/requests/test_requests_60.json --device cuda:0"
  echo
fi
...
if [[ "$BACKEND" == "all" || "$BACKEND" == "actionseq" ]]; then
  ...
  echo "example:"
  echo "  python scripts/run_eval.py --backend actionseq --requests-file artifact/data/requests/test_requests_60.json --device cuda:0"
  echo
fi
